  ...
  if (odDataDir) {
    ...
    if (!isAbs) {
      throw new PackagedPathAccessError(
        [
          "AIWP Design's packaged runtime requires OD_DATA_DIR to be an absolute path.",
          "",
          `Configured value: ${odDataDir}`,
          "",
          "Set OD_DATA_DIR to an absolute path (for example, C:\\\\Users\\\\You\\\\AIWP Design on Windows or /Users/you/AIWP Design on macOS/Linux) and relaunch AIWP Design.",
        ].join("\n"),
        { title: "AIWP Design cannot start with this OD_DATA_DIR" },
      );
    }
    ...
    if (scopedNamespace) {
      if (scopedNamespace !== namespace) {
        throw new PackagedPathAccessError(
          [
            "AIWP Design's packaged runtime requires OD_DATA_DIR to target the active namespace.",
            "",
            `Configured value: ${odDataDir}`,
...
export function resolvePackagedNamespacePaths(
  ...
  // `~/Library/Application Support/AIWP Design Prerelease/` on mac. Custom
