...
export async function ensureMcpDaemonUrl(
  ...
  if (plan.action === "none") {
    ...
    throw new Error(
      `The registered AIWP Design runtime is unavailable and cannot be launched (${plan.reason}).`,
    );
  }
  ...
  throw new Error(
    `AIWP Design was launched headlessly but its daemon did not become ready within ${timeoutMs}ms.`,
  );
}
