...
// external agents (hermes-agent, openclaw, etc.) can drive AIWP Design
...
Options:
  ...
  --daemon-url <url>       Override the AIWP Design daemon HTTP base.`);
}
...
Install or repair AIWP Design's bundled connection component in the user's
official DeepSeek Harness installation. The dsh CLI itself is not installed
or upgraded by AIWP Design.
...
function printRootHelp() {
  ...
  od plugin open-design-pr <folder>
      Push a community-catalog branch and open the AIWP Design PR form.
  ...
  od share <open-design|url> [options]
      Build localized social-share targets for the AIWP Design repo or a
  ...
  od mcp [--daemon-url <url>]
      ...
      running AIWP Design daemon. Wire it into a coding agent
      (Claude Code, Cursor, VS Code, Zed, Windsurf) in another repo
      to pull files from a local AIWP Design project and create
...
Options:
  --daemon-url <url>   AIWP Design daemon HTTP base.
...
Runs Tavily-backed shallow research through the local AIWP Design daemon.
  ...
  if (code === 'EPERM' || code === 'ENETUNREACH') {
    console.error(
      'hint: outbound connect was denied by a sandbox. If you launched ' +
        ...
        'network policy. The AIWP Design daemon itself is unaffected - it can be ' +
...
Common options:
  ...
  --quality <tier>          AIWP Design Cloud images only: published quality tier
  ...
  --resolution <res>        AIWP Design Cloud images only: published output resolution
...
tool calls to a running AIWP Design daemon. Wire it into a coding agent
in another repo so the agent can pull files from a local AIWP Design
project and create project-scoped artifacts without exporting a zip
every iteration.

Options:
  --daemon-url <url>   AIWP Design daemon HTTP base URL. Resolution
                       ...
                       survive an AIWP Design restart.
...
Tools exposed:
  list_projects                  list every AIWP Design project
...
user has open in AIWP Design; get_artifact and get_file additionally
...
→ MCP server in the AIWP Design app. The daemon must be running locally
...
Register AIWP Design's stdio MCP server into a coding agent's own config.

Agents:
  ${AGENT_SLUGS.join(' ')}

Options:
  --uninstall, --remove   Remove the AIWP Design MCP server instead.
...
Wraps GitHub CLI auth for AIWP Design registry publishing. The token stays in gh.`);
    return;
  }
...
Shows the GitHub account gh will use for AIWP Design registry publishing.`);
    return;
  }
  const host = typeof flags.host === 'string' ? flags.host : 'github.com';
...
Common options:
  --daemon-url <url>   AIWP Design daemon HTTP base (default OD_DAEMON_URL, OD_SIDECAR_IPC_PATH discovery, or http://127.0.0.1:7456).
  ...
  switch (sub) {
    ...
    case 'login': {
      ...
      console.log(`[marketplace login] authenticating gh for ${host}. Tokens stay in gh, not AIWP Design.`);
...
// these so a code agent can drive AIWP Design end-to-end without
...
Imports a local project directory as an editable AIWP Design design system.
...
Imports a public GitHub repository as an editable AIWP Design design system.
...
Imports a shadcn registry item as an AIWP Design design system.
...
async function runWhatsNew(args) {
  ...
  const data = await resp.json();
  if (flags.json) return process.stdout.write(JSON.stringify(data, null, 2) + '\n');
  console.log(`AIWP Design ${data?.version ?? 'unknown'}`);
