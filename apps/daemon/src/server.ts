...
export function createAgentRuntimeToolPrompt(
  ...
  return [
    ...
    '- `OD_HYPERFRAMES_BIN` is the absolute path to AIWP Design\'s pinned HyperFrames CLI. Run lightweight commands through `OD_NODE_BIN`; use `"$OD_NODE_BIN" "$OD_BIN" media scaffold` for composition setup and never use a user-level `npx` cache.',
    '- `OD_BIN` is the absolute path to the AIWP Design CLI script. On POSIX shells run wrappers with `"$OD_NODE_BIN" "$OD_BIN" tools ...`; do not call bare `od`, which may resolve to the system octal-dump command on Unix-like systems.',
...
export async function startServer({
  ...
  if (apiTokenAuthEnabled) {
    ...
    app.use((req, res, next) => {
      ...
      return res.status(401).type('text/plain').send(
        'AIWP Design authentication required. Use username "open-design" and OD_API_TOKEN as the password.',
      );
    });
  }
  ...
  const startChatRun = async (chatBody, run) => {
    ...
    // AIWP Design relays them to the agent so the model can call those tools.
    ...
    if (def.id === 'codex') {
      ...
      // When AIWP Design leaves model selection at `default`, Codex resolves
  ...
  orbitService.setRunHandler(async ({
    ...
    design.runs.start(run, () => startChatRun({
      ...
      systemPrompt: [
        ...
        systemPrompt,
        'You are Orbit, an autonomous activity-summary agent inside AIWP Design.',
