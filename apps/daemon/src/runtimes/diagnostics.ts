...
export function buildVersionDiagnostic(
  ...
  if (!version) {
    return {
      ...
      severity: 'error',
      message: `${def.name} was found, but AIWP Design could not verify its version.`,
  ...
  return {
    ...
    severity: 'warning',
    message: `${def.name} ${version} has not been tested with this AIWP Design build.`,
...
export function buildCompatibilityDiagnostic(
  ...
  return {
    ...
    severity: 'error',
    message: `${def.name} is installed, but its AIWP Design profile is missing or incompatible.`,
    detail: 'Install the pinned AIWP Design profile bundle in Harness profile `open-design`, then rescan.',
    fixActions: [
      { kind: 'openDocs' },
      { kind: 'rescan' },
    ],
