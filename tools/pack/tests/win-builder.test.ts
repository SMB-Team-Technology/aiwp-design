...
function createPaths(root: string): WinPaths {
  ...
  return {
    ...
    blockmapPath: join(namespaceRoot, "builder", "AIWP Design-second-setup.exe.blockmap"),
    ...
    exePath: join(namespaceRoot, "builder", "AIWP Design-second.exe"),
    installDir: join(namespaceRoot, "runtime", "install", "AIWP Design"),
    installedExePath: join(namespaceRoot, "runtime", "install", "AIWP Design", "AIWP Design.exe"),
    ...
    launcherPayloadPath: join(namespaceRoot, "payload", "AIWP Design-second-payload.7z"),
    ...
    setupPath: join(namespaceRoot, "builder", "AIWP Design-second-setup.exe"),
    setupZipPath: join(namespaceRoot, "builder", "AIWP Design-second-portable.zip"),
    ...
    uninstallerPath: join(namespaceRoot, "runtime", "install", "AIWP Design", "Uninstall.exe"),
...
describe("materializeCachedUnpackedForInstaller", () => {
  it("overwrites cached packaged config and app package version", async () => {
    ...
    try {
      ...
      await writeFile(join(cachedUnpackedRoot, "AIWP Design.exe"), await createVersionedExecutable("0.5.0-beta.1"));
      await writeFile(
      ...
      await expect(readWinExecutableVersionSnapshot(join(paths.unpackedRoot, "AIWP Design.exe"))).resolves.toMatchObject({
