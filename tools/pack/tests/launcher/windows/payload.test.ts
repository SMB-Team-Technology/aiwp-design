...
function createWinPaths(root: string, namespace: string): WinPaths {
  ...
  return {
    ...
    blockmapPath: join(namespaceRoot, "builder", "AIWP Design-release-beta-win-setup.exe.blockmap"),
    ...
    exePath: join(namespaceRoot, "builder", "AIWP Design-release-beta-win.exe"),
    installDir: join(namespaceRoot, "runtime", "install", "AIWP Design Beta"),
    installedExePath: join(namespaceRoot, "runtime", "install", "AIWP Design Beta", "AIWP Design.exe"),
    ...
    launcherPayloadPath: join(namespaceRoot, "payload", "AIWP Design-release-beta-win-payload.7z"),
    ...
    setupPath: join(namespaceRoot, "builder", "AIWP Design-release-beta-win-setup.exe"),
    setupZipPath: join(namespaceRoot, "builder", "AIWP Design-release-beta-win-portable.zip"),
    ...
    uninstallerPath: join(namespaceRoot, "runtime", "install", "AIWP Design Beta", "Uninstall.exe"),
    ...
    unpackedExePath: join(namespaceRoot, "builder", "win-unpacked", "AIWP Design.exe"),
...
async function writeFakeWinUnpackedApp(root: string, namespace: string, version: string): Promise<{
  ...
  await writeFile(join(paths.unpackedRoot, "AIWP Design.exe"), "fake executable\n", "utf8");
  await writeFile(
  ...
  return {
    builtApp: {
      ...
      executablePath: join(paths.unpackedRoot, "AIWP Design.exe"),
      source: "namespace",
...
describe("tools-pack Windows launcher payload archives", () => {
  it("builds a channel- and namespace-scoped payload manifest", () => {
    expect(buildWinLauncherPayloadManifest({
      ...
      entry: {
        cwd: "payload",
        executable: "payload/AIWP Design.exe",
      },
      namespace: "release-beta-win",
      payloadRoot: "payload",
      platform: "win32",
  ...
  it.skipIf(process.platform !== "win32")("creates a Windows payload 7z with bootstrap-readable contents", async () => {
    ...
    try {
      ...
      expect(manifest.entry.executable).toBe("payload/AIWP Design.exe");
      expect(manifest.version).toBe(version);
      await expectPathExists(join(extractRoot, "payload", "AIWP Design.exe"));
