...
describe("tools-pack mac launcher payload archives", () => {
  it("builds a channel- and namespace-scoped payload manifest", () => {
    ...
    expect(buildMacLauncherPayloadManifest({
      ...
      appBundleName: "AIWP Design Beta.app",
      channel: "beta",
      entry: {
        cwd: "payload/AIWP Design Beta.app",
        executable: "payload/AIWP Design Beta.app/Contents/MacOS/AIWP Design Beta",
      },
      namespace: "release-beta",
      payloadRoot: "payload",
      platform: "darwin",
  ...
  it.skipIf(process.platform !== "darwin")("creates a payload zip with bootstrap-readable contents", async () => {
    ...
    try {
      ...
      expect(manifest.appBundleName).toBe("AIWP Design Beta.app");
      expect(manifest.entry.executable).toBe("payload/AIWP Design Beta.app/Contents/MacOS/AIWP Design Beta");
