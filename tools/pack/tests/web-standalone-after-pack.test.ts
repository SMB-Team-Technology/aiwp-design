...
async function writeHyperframesRuntimeFixture(options: {
  ...
  const bundledNodePath = options.platformName === "win32"
    ? join(appPath, "AIWP Design.exe")
    : join(appPath, "Contents", "MacOS", "AIWP Design");
...
async function runFixture(options: {
  ...
  const resourcesRoot = platformName === "darwin"
    ? join(appOutDir, "AIWP Design.app", "Contents", "Resources")
    : join(appOutDir, "resources");
  const appPath = join(appOutDir, "AIWP Design.app");
  ...
  if (platformName === "darwin" && options.writeMacCodeBundleFixture) {
    ...
    await mkdir(join(frameworksRoot, "AIWP Design Helper.app"), { recursive: true });
  }
  ...
  try {
    await runWebStandaloneAfterPack({
      ...
      packager: { appInfo: { productFilename: "AIWP Design" } },
    });
  } catch (error) {
    ...
    try {
      ...
      expect(signedTargets).toEqual(
        expect.arrayContaining([
          ...
          expect.stringMatching(/AIWP Design Helper\.app$/),
          expect.stringMatching(/AIWP Design\.app$/),
        ]),
      );
