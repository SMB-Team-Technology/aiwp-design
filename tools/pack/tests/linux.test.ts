  ...
  it("ignores the desktop AppImage identity marker and reads only the headless marker", async () => {
    ...
    try {
      ...
      await writeFile(
        markerPath,
        `${JSON.stringify({
          appPath: "/tmp/AIWP-Design.AppImage",
...
describe("stopPackedLinuxApp", () => {
  linuxOnlyIt("treats a direct AppRun-launched Electron process as owned", async () => {
    ...
    const executablePath = join(appDir, "AIWP Design");
    ...
    try {
      ...
      await writeFile(join(config.roots.output.appBuilderRoot, "AIWP-Design.direct-apprun.AppImage"), "", "utf8");
...
`;
  ...
  it("substitutes all @@TOKEN@@ placeholders", () => {
    const out = renderDesktopTemplate(template, {
      namespace: "default",
      execPath: "/home/u/.local/bin/AIWP-Design.default.AppImage",
      iconName: "open-design-default",
    });
    expect(out).toContain("Name=AIWP Design (default)");
    expect(out).toContain(
      "Exec=env -u ELECTRON_RUN_AS_NODE OD_PACKAGED_NAMESPACE=default /home/u/.local/bin/AIWP-Design.default.AppImage --appimage-extract-and-run %U",
    );
...
describe("renderLinuxAppImageAppRun", () => {
  it("unsets ELECTRON_RUN_AS_NODE before execing the Electron binary", () => {
    ...
    expect(out).toContain('BIN="$APPDIR/AIWP Design"');
  });
  ...
  linuxOnlyIt("exports APPIMAGE fallback to the execed Electron process", async () => {
    ...
    const electronPath = join(appDir, "AIWP Design");
...
describe("inspectPackedLinuxApp", () => {
  ...
  it("allows desktop inspect eval and screenshot options when headless is omitted", async () => {
    ...
    requestJsonIpcMock
      ...
      .mockResolvedValueOnce({ ok: true, value: "AIWP Design" })
    ...
    expect(result).toEqual({
      eval: { ok: true, value: "AIWP Design" },
...
describe("matchesAppImageProcess", () => {
  const installPath = "/home/u/.local/bin/AIWP-Design.default.AppImage";
  ...
  it("matches direct AppRun fallback when APPIMAGE points to the sibling AppRun", () => {
    const ok = matchesAppImageProcess(
      {
        pid: 1234,
        executable: "/tmp/appimage_extracted_fe548e54/AIWP Design",
